...
function hbbtvlib_initialize() {
	try {
		int_createOipfObjs();
		if (int_appMan && typeof(int_appMan.getOwnerApplication) == "function")
			int_app = int_appMan.getOwnerApplication(document);
		else {
...
/**
 * Old implementation has problems.
 * Array of OIPF DAE embedded objects. Access Key is the mime-type without "application/".
 *
 * Add an embedded object with the application/oipfApplicationManager type and and id attribute as defined below.
 */
var int_appMan = undefined;
var int_appManId = "appMan";
...
function int_createOipfObjs() {
	if (int_appMan) return;
	// Look for existing objects
	int_appMan = document.getElementById(int_appManId) ||
							 oipfObjectFactory.createApplicationManagerObject();
};
